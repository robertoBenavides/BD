...
string fixCadsize(string val,int tam) {
    string finalstring=string(val.begin(), val.begin() + tam);
                ...
                if (string(value.begin(), value.begin() + pos) == "set") {
                    ...
                    for (string p : parametros) {
                        ...
                        if (tb.colums[columnumber].tamanio != "0") {
                            ...
                            data[indexdata][columnumber] = charval;
                        }
                        else {
...
Tabla tableexist(string tbname) {
    ...
    //cout << "No se enccontró la tabla \"" + tbname + "\n" << endl;
    return Tabla();
}
void insertValue(string val) {
    size_t pos = (val.find(" ")>val.find("("))?val.find("("): val.find(" ");
...
int main()
{
    ...
    cargardata();
    system("cls");
    cout << "todo comando termina en ';'"<<endl;
    bool running = true;
    while (running) {
        string comando = readComand();
        size_t pos = comando.find(" ");
